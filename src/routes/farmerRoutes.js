...
farmerRouter.post(
  "/register",
  validateRequest(farmerValidationSchema),
  farmerRegister
);
farmerRouter.post(
  "/register/contact",
  validateRequest(farmerContactRegisterSchema),
  farmerRegisterWithContact
);
//farmerRouter.get('/phoneNo', farmerLoginOtp); // specific
farmerRouter.post(
  "/login",
  validateRequest(loginFarmerValidation),
  farmerLogin
); // specific
...
farmerRouter.post("/by-ids", authenticateJWT, getFarmersByIds);
